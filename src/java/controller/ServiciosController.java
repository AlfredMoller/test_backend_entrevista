...
public class ServiciosController {
    ...
    public Response listarPagosPorFecha(JsonObject jsonObject) {
        try {
            ...
            List<DeudasServicios> pagos = deufacades.listarPagosPorFecha(idUsuario, nisOCedula, fechaInicio, fechaFin, logId, page, size);
            int totalPagos = deufacades.contarPagos(idUsuario, nisOCedula, fechaInicio, fechaFin, logId); // Método adicional para contar todos los pagos
            ...
            if (pagos.isEmpty()) {
                ...
                pagos.forEach(pago -> {
                    arrayBuilder.add(Json.createObjectBuilder()
                        .add("id_pago", pago.getIdDeuda())
                        .add("monto_pago", pago.getMontoAbonado())
                        .add("fecha_pago", pago.getFechaVencimiento().toString())
